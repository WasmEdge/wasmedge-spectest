...
* `component-model-` prefixed folders: The tests for the [Component Model](https://github.com/WebAssembly/component-model) proposal, mirroring the folders under the [`test`](https://github.com/WebAssembly/component-model/tree/main/test) directory of the spec repository:
  ...
  * Six of these tests are listed in the spec repository's own `test/nyi.txt` as not implemented by any engine yet: `component-model-validation/kebab`, `component-model-binary/binary`, `component-model-values/post-return`, `component-model-async/big-interleaving-test`, `component-model-async/forward`, and `component-model-async/cancel-not-delivered`.
...
The `.wast` files are copied from the [`test`](https://github.com/WebAssembly/component-model/tree/main/test) directory of the spec repository (commit `5b724da`), modified as described below, and converted into `.json` and `.wasm` files with the `json-from-wast` command of [wasm-tools](https://github.com/bytecodealliance/wasm-tools). Tests changed since spec commit `8b5c200` use wasm-tools built from `main` (commit `3663a40`) for the unreleased `stream.forward` and `future.forward` built-ins; the others use wasm-tools 1.256.0.

`(component definition ...)` and `(component instance ...)` become the `"module_definition"` and `"module_instance"` commands above, and `assert_trap` on component instantiation becomes `"assert_uninstantiable"`. Component-level values in `"args"` and `"expected"` use the WIT type names:

* `{"type": "u32", "value": "42"}`, `{"type": "bool", "value": true}`, `{"type": "char", "value": "Z"}`, `{"type": "string", "value": "hi"}`
* `{"type": "list", "value": [...]}`, `{"type": "tuple", "value": [...]}`, `{"type": "record", "value": [["field-name", {...}], ...]}`
* `{"type": "variant", "value": {"case": "s", "payload": {...}}}`, `{"type": "enum", "value": "red"}`, `{"type": "flags", "value": ["a", "c"]}`
* `{"type": "option", "value": {...}}` (`null` for `none`), `{"type": "result", "value": {"Ok": {...}}}` (or `{"Err": {...}}`)

#### Component-Model Trap Message Changes

The expected messages are adjusted to WasmEdge's diagnostics (<https://github.com/WasmEdge/WasmEdge/pull/4666>). WasmEdge reports fixed error strings and matches by prefix, so the `text` of an assertion has to *start with* the WasmEdge error string:

* Variable parts are removed, e.g. `` unknown instance: failed to find name `$i` `` -> `unknown instance`, `expected func, found component` -> `expected func`, `` import name `a` conflicts with previous name `a` `` -> `import name conflicts with previous name`.
* A tail after the WasmEdge error string is kept, e.g. `` handle index 1 used with the wrong type, ... `` -> `used with the wrong type, ...`.
* Messages with no matching wording are replaced by the WasmEdge error string, e.g. `expected 0 parameters, found 1` -> `type mismatch in function type`, and `` invalid leading byte (0x44) for component defined type `` -> `malformed defined type`.
* The `wasm trap:` prefix of wasmtime runtime traps is dropped.
* Empty expected texts of `assert_malformed` in `component-model-binary/binary/binary.wast` are filled in with the WasmEdge message.
* `section out of order` in `component-model-binary/binary/binary.wast` line 208 is changed to `unexpected content after last section`, the message used by the WASM spec tests.
* `component-model-binary/binary/binary.wast` line 1115 tests the unallocated opcode `0x30` instead of `0x2e`, which `Binary.md` now assigns to `stream.forward` (used by `component-model-async/forward`). Lines 1173 and 1182 expect `non-zero reserved byte` like lines 1206 and 1228, because `Binary.md` replaced the former `cancel?` flag with a reserved `0x00` byte.

The reference messages are the ones from `wasm-tools validate --features all` (1.256.0) for loading and validation, and from `wasmtime wast` (46.0.0) for linking and runtime.
...
* `component-model-async/forward/forward.wast` line 1455: `` `future.forward` requires a future type `` -> `` requires a future type ``
* `component-model-async/forward/forward.wast` line 1449: `` `stream.forward` requires a stream type `` -> `` requires a stream type ``
* `component-model-async/forward/forward.wast` line 993, 1011: `` handle index 1 used with the wrong type `` -> `` used with the wrong type ``
* `component-model-async/forward/forward.wast` line 991, 1013: `` handle index 2 used with the wrong type `` -> `` used with the wrong type ``
...
* `component-model-binary/binary/binary.wast` line 533: `` core instance 0 has no export named `f` `` -> `` unknown export ``
* `component-model-binary/binary/binary.wast` line 1642: `` expected a version header for a component `` -> `` unknown binary version ``
* `component-model-binary/binary/binary.wast` line 217: `` expected a version header for a module `` -> `` unknown binary version ``
* `component-model-binary/binary/binary.wast` line 518: `` instance 0 has no export named `t` `` -> `` unknown export ``
* `component-model-binary/binary/binary.wast` line 1173, 1182: `` invalid boolean value `` -> `` non-zero reserved byte ``
* `component-model-binary/binary/binary.wast` line 955: `` invalid leading byte `` -> `` malformed module type ``
* `component-model-binary/binary/binary.wast` line 1426: `` invalid leading byte (0x0) for component external kind `` -> `` integer representation too long ``
...
* `component-model-binary/binary/binary.wast` line 933: `` invalid leading byte (0x0) for outer alias kind `` -> `` malformed sort ``
* `component-model-binary/binary/binary.wast` line 943: `` invalid leading byte (0x0) for outer alias target `` -> `` malformed alias target ``
* `component-model-binary/binary/binary.wast` line 1136: `` invalid leading byte (0x1) for canonical function lift `` -> `` malformed canonical ``
* `component-model-binary/binary/binary.wast` line 1145: `` invalid leading byte (0x1) for canonical function lower `` -> `` malformed canonical ``
...
* `component-model-binary/binary/binary.wast` line 774: `` invalid leading byte (0x1) for number of results `` -> `` malformed defined type ``
* `component-model-binary/binary/binary.wast` line 634: `` invalid leading byte (0x1) for zero byte required `` -> `` malformed variant type ``
...
* `component-model-binary/binary/binary.wast` line 784: `` invalid leading byte (0x2) for component function results `` -> `` malformed defined type ``
...
* `component-model-binary/binary/binary.wast` line 1575: `` invalid leading byte (0x2) for optional component export type `` -> `` malformed defined type ``
* `component-model-binary/binary/binary.wast` line 1404: `` invalid leading byte (0x2) for type bound `` -> `` invalid wasm grammar ``
* `component-model-binary/binary/binary.wast` line 1117: `` invalid leading byte (0x2e) for canonical function `` -> `` malformed canonical `` (for the `0x30` opcode, see above)
* `component-model-binary/binary/binary.wast` line 431: `` invalid leading byte (0x3) for alias `` -> `` malformed alias target ``
* `component-model-binary/binary/binary.wast` line 1379: `` invalid leading byte (0x3) for component name `` -> `` malformed name ``
* `component-model-binary/binary/binary.wast` line 875: `` invalid leading byte (0x3) for component or instance type declaration `` -> `` malformed defined type ``
* `component-model-binary/binary/binary.wast` line 1393: `` invalid leading byte (0x3) for name option `` -> `` malformed name ``
* `component-model-binary/binary/binary.wast` line 621: `` invalid leading byte (0x3e) for component defined type `` -> `` malformed defined type ``
* `component-model-binary/binary/binary.wast` line 923: `` invalid leading byte (0x4) for type definition `` -> `` invalid wasm grammar ``
* `component-model-binary/binary/binary.wast` line 1126: `` invalid leading byte (0x43) for canonical function `` -> `` malformed canonical ``
* `component-model-binary/binary/binary.wast` line 612: `` invalid leading byte (0x44) for component defined type `` -> `` malformed defined type ``
* `component-model-binary/binary/binary.wast` line 863: `` invalid leading byte (0x5) for component or instance type declaration `` -> `` malformed defined type ``
...
* `component-model-binary/binary/binary.wast` line 1415: `` invalid leading byte (0x6) for component external kind `` -> `` invalid wasm grammar ``
* `component-model-binary/binary/binary.wast` line 1608: `` invalid leading byte (0x6) for component external kind `` -> `` malformed sort ``
...
* `component-model-binary/binary/binary.wast` line 603: `` invalid leading byte (0x62) for component defined type `` -> `` malformed defined type ``
* `component-model-binary/binary/binary.wast` line 1108: `` invalid leading byte (0x7) for canonical function `` -> `` malformed canonical ``
* `component-model-binary/binary/binary.wast` line 1163: `` invalid leading byte (0xa) for canonical option `` -> `` unknown canonical option ``
* `component-model-binary/binary/binary.wast` line 1463, 1477: `` is not a valid extern name `` -> `` not in kebab case ``
* `component-model-binary/binary/binary.wast` line 750: `` is not valid at this time `` -> `` `stream<char>` is not valid at this time ``
* `component-model-binary/binary/binary.wast` line 208: `` section out of order `` -> `` unexpected content after last section ``
* `component-model-binary/binary/binary.wast` line 741: `` type index 0 is not a resource type `` -> `` not a resource type ``
* `component-model-binary/binary/binary.wast` line 165: `` unexpected end-of-file `` -> `` integer representation too long ``
* `component-model-binary/binary/binary.wast` line 50, 90, 1437: `` unexpected end-of-file `` -> `` length out of bounds ``
...
* `component-model-validation/kebab/kebab.wast` line 100: `` `1` is not in kebab case `` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 116: `` `DOWn` is not in kebab case `` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 104: `` `GonnA` is not in kebab case `` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 53: `` `HtTp` is not in kebab case `` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 108: `` `NevEr` is not in kebab case `` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 45: `` `TyPeS` is not in kebab case `` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 49: `` `WaSi` is not in kebab case `` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 61, 65, 69: ``` `` is not in kebab case ``` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 37: `` `aBc` is not in kebab case `` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 112: `` `lET` is not in kebab case `` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 57: `` `wasi/http` is not in kebab case `` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 142: `` export name `A` conflicts with previous name `a` `` -> `` conflicts with previous export name ``
* `component-model-validation/kebab/kebab.wast` line 130: `` export name `A` conflicts with previous name `a` `` -> `` export name conflicts with previous name ``
* `component-model-validation/kebab/kebab.wast` line 148: `` export name `FOO-bar-BAZ` conflicts with previous name `foo-BAR-baz` `` -> `` conflicts with previous export name ``
* `component-model-validation/kebab/kebab.wast` line 124: `` export name `a` conflicts with previous name `a` `` -> `` export name conflicts with previous name ``
* `component-model-validation/kebab/kebab.wast` line 158: `` import name `FOOBAR` conflicts with previous name `foo-bar` `` -> `` import name conflicts with previous name ``
* `component-model-validation/kebab/kebab.wast` line 173: `` import name `[method]foo-bar.foobar` conflicts with previous name `foo-bar` `` -> `` import name conflicts with previous name ``
* `component-model-validation/kebab/kebab.wast` line 168: `` import name `[static]foo-bar.FO-ob-AR` conflicts with previous name `foo-bar` `` -> `` import name conflicts with previous name ``
* `component-model-validation/kebab/kebab.wast` line 136: `` import name `a` conflicts with previous name `A` `` -> `` conflicts with previous import name ``
* `component-model-validation/kebab/kebab.wast` line 163: `` import name `foob-ar` conflicts with previous name `foo-bar` `` -> `` import name conflicts with previous name ``
* `component-model-validation/kebab/kebab.wast` line 153: `` import name `foobar` conflicts with previous name `foo-bar` `` -> `` import name conflicts with previous name ``
* `component-model-validation/kebab/kebab.wast` line 77, 89: `` is not a valid extern name `` -> `` not in kebab case ``
* `component-model-validation/kebab/kebab.wast` line 73, 81, 85, 93: `` is not a valid extern name `` -> `` not lowercase in package name/namespace ``
* `component-model-validation/kebab/kebab.wast` line 17, 21, 25, 29, 33, 41: `` is not in kebab case `` -> `` not in kebab case ``
